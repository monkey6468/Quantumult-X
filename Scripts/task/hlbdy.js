...
[task_local]
25 15,23 * * * https://raw.githubusercontent.com/1019459067/Quantumult-X/master/Scripts/task/hlbdy.js, tag=黑料不打烊, enabled=true
*/
...
!(async () => {
    ...
    const msg = hotList.map(item => `【${item.title}(${item.date})】\n${item.link}\n`)
        .join('');
    $.msg(scriptName, '最新黑料', msg, {
        "media-url": url + '/static/pc/icons/icon_512x512.820c9b.png?v=1'
    });
    $notify(scriptName, '最新黑料', msg);
})()
.catch((e) => $.logErr(e))
    .finally(() => $.done());
// 获取最新地址
